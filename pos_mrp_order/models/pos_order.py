...
class PosOrder(models.Model):
    ...
    def create_picking(self):
        ...
        partner = self.env['res.partner'].search([('name','ilike','Admin')],limit=1)
        ...
        for order in self:
            ...
            for line in order.lines.filtered(lambda l: l.product_id.type in ['product', 'consu'] and not float_is_zero(l.qty, precision_rounding=l.product_id.uom_id.rounding)):
                ...
                import ast
                for extra_line in ast.literal_eval(line.extra_notes):
                    for key in extra_line.keys():
                        product_id = self.env['product.product'].search([('name','=',key)])
                        moves |= Move.create({
                            'name': line.name,
